...
for e in [1e-1, 5e-1, 1e-2, 5e-2, 1e-3, 5e-3, 1e-4]:
    ...
    tp0.pull(source=0, target=2, data=cell_type_key, subset=['Neutrophil'])
    adata.obs['pull_Neutrophil'] = adata.obs['pull'].copy()
    tp0.pull(source=0, target=2, data=cell_type_key, subset=['Monocyte'])
    adata.obs['pull_Monocyte'] = adata.obs['pull'].copy()
    adata.obs['moscot_fate_bias'] = adata.obs['pull_Neutrophil'] / (adata.obs['pull_Neutrophil'] + adata.obs['pull_Monocyte'])
    moscot_pred.append(adata.obs['moscot_fate_bias'].loc[adata.obs.loc[adata.obs[time_key] == 0].index].copy())
    
    tp0.pull(source=1, target=2, data=cell_type_key, subset=['Neutrophil'])
    adata.obs['pull_Neutrophil'] = adata.obs['pull'].copy()
    tp0.pull(source=1, target=2, data=cell_type_key, subset=['Monocyte'])
    adata.obs['pull_Monocyte'] = adata.obs['pull'].copy()
    adata.obs['moscot_fate_bias'] = adata.obs['pull_Neutrophil'] / (adata.obs['pull_Neutrophil'] + adata.obs['pull_Monocyte'])
    ...
    pd.DataFrame(moscot_pred).to_csv('../pygot_data/03_cellfate/hpsc/moscot_v4.0_{}.csv'.format(e))
